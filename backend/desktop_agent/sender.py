...
class TelemetrySender:
    ...
    def send_telemetry(self, payload: Dict[str, Any]) -> Tuple[bool, Dict[str, Any]]:
        ...
        try:
            res = requests.post(self.backend_url, json=payload, headers=self.headers, timeout=1.5)
    ...
    def flush_offline_queue(self):
        ...
        for queued_item in to_flush:
            try:
                requests.post(self.backend_url, json=queued_item, headers=self.headers, timeout=1.5)
            except Exception:
